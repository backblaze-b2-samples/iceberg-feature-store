...
def get_s3_client():
    return boto3.client(
        ...
        config=Config(
            ...
            user_agent_extra="b2ai-iceberg-feature-store",
        ),
    )
...
def get_object_bytes(key: str) -> bytes:
    """Download an object's raw bytes from B2. Raises RuntimeError on failure."""
    client = get_s3_client()
    try:
...
def get_prefix_size(prefix: str) -> dict:
    """Sum object count + total bytes under a prefix (paginated).

    Used for the 'warehouse size on B2' dashboard metric (prefix=warehouse/)
    and for raw/exports sizing. Raises RuntimeError on S3 failure.
    """
    client = get_s3_client()
    count = 0
    total = 0
    ...
    try:
        ...
        raise RuntimeError(f"B2 prefix-size query failed for '{prefix}': {e}") from e
    return {
        "object_count": count,
