...
export async function ingestBatch(rows?: number | null) {
  return apiFetch<IngestResponse>("/ingest/batch", {
    method: "POST",
...
export async function ingestRawFile(key: string) {
  return apiFetch<IngestResponse>(`/ingest/raw?key=${encodeURIComponent(key)}`, {
    method: "POST",
  });
}

export async function evolveSchema() {
  return apiFetch<EvolveResponse>("/ingest/evolve-schema", { method: "POST" });
}
...
export async function getSnapshots() {
  return apiFetch<SnapshotInfo[]>("/tables/snapshots");
}

export async function getSchemaVersions() {
  return apiFetch<SchemaVersion[]>("/tables/schema");
}

export async function getWarehouseFiles(limit = 1000) {
...
export async function runQuery(req: QueryRequest) {
...
export async function exportSlice(req: ExportRequest) {
  return apiFetch<ExportResponse>("/query/export", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(req),
  });
...
export async function getDashboardActivity(limit = 20) {
  return apiFetch<ActivityEntry[]>(`/dashboard/activity?limit=${limit}`);
}
